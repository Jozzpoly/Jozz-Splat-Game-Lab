...
export class SurveyController {
  constructor({ canvas, setCamera, onCameraChanged, target, position, fovDeg = 58 }) {
    this.canvas = canvas;
  ...
  #derive(position, target) {
    ...
    const radius = Math.max(this.minRadius, Math.hypot(dx, dy, dz));
  ...
  #apply(reason = 'camera') {
    ...
    this.onCameraChanged(reason);
  }

  setFocusResolver(resolver) { this.focusResolver = typeof resolver === 'function' ? resolver : null; }
  ...
  async focusAtCursor() {
    ...
    const x = Number.isFinite(this.pointerX) && this.#inside(this.pointerX, this.pointerY) ? this.pointerX : rect.left + rect.width * 0.5;
    const y = Number.isFinite(this.pointerY) && this.#inside(this.pointerX, this.pointerY) ? this.pointerY : rect.top + rect.height * 0.5;
  ...
  onPointerMove(event) {
    if (event.target === this.canvas && this.#inside(event.clientX, event.clientY)) {
      this.pointerX = event.clientX;
      this.pointerY = event.clientY;
    }
  ...
  onWheel(event) {
    ...
    const nextRadius = Math.max(this.minRadius, previousRadius * Math.exp(boundedDelta * speed));
  ...
  onKeyDown(event) {
    ...
    if (key === 'f') { event.preventDefault(); void this.focusAtCursor(); }
    else if (event.key === 'Home') { event.preventDefault(); this.fit(); }
    else if (key === 'r') { event.preventDefault(); this.reset(); }
  }
