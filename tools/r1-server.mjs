...
const labRoot = join(repoRoot, 'performance-lab');
const sourcePath = process.argv[2] ? resolve(process.argv[2]) : '';

function fail(message, code) {
  console.error(`R1 server: ${message}`);
  process.exit(code);
}
...
if (sourceStat.size !== SOURCE_BYTES) fail(`zły rozmiar źródła (${sourceStat.size}).`, 3);
console.log('R1: potwierdzam SHA-256 źródła…');
const sourceSha = await sha256File(sourcePath);
if (sourceSha !== EXPECTED_SOURCE_SHA) fail(`SHA-256 PLY nie pasuje: ${sourceSha}`, 4);
...
if (foregroundHeader.length !== HEADER_BYTES) fail('foreground header zmienił długość.', 6);
...
function serveStatic(requestPath, res, headOnly) {
  ...
  const rel = pathRelative(resolve(labRoot), fullPath);
...
const server = createServer((req, res) => {
  ...
  return serveStatic(url.pathname, res, headOnly);
});
...
const url = `http://127.0.0.1:${port}/?profile=quiet&backend=best`;
console.log('');
console.log('============================================================');
console.log('Jozz Splat Game Lab — R1 THERMAL RENDER BUDGET');
console.log('============================================================');
console.log(`Source: VERIFIED ${sourceSha.slice(0, 12)}…`);
console.log(`LAB: ${url}`);
console.log('Default: Quiet 60 + Best backend');
console.log('');
