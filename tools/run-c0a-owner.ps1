...
try {
    ...
    if ($extension -eq '.zip') {
        ...
        if ($archiveInfo.Length -ne $ExpectedArchiveBytes) { throw "To nie jest zweryfikowany ZIP F0." }
        ...
        if ($archiveHash -ne $ExpectedArchiveSha256) { throw "ZIP nie spelnia F0 SHA-256 contract." }
        $extractDir = Join-Path ([System.IO.Path]::GetTempPath()) ('JozzSplatGameLab_C0a_' + [Guid]::NewGuid().ToString('N'))
        ...
        if ($plyFiles.Count -ne 1) { throw "C0a oczekuje dokladnie jednego PLY; znaleziono: $($plyFiles.Count)." }
    ...
    if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
        throw 'Nie znaleziono Node.js. Nie instaluj nic samodzielnie; wyslij mi screenshot.'
    }

    Write-Host ''
    Write-Host 'Uruchamiam C0a Collision Inspector...'
    Write-Host 'To jest niemetryczny test ksztaltu geometrii. Nie uruchamia Box3D i nie udaje metrow.'
    Write-Host ''
    & node tools/c0a-server.mjs $sourcePath
    exit $LASTEXITCODE
}
catch {
    Write-Host ''
    ...
    Show-ErrorBox ("C0a: FAIL`n`n" + $_.Exception.Message + "`n`nNie naprawiaj tego sam. Wyslij mi screenshot komunikatu.")
    exit 1
}
finally {
