...
try {
    ...
    if ($extension -eq '.zip') {
        $extractDir = Join-Path ([System.IO.Path]::GetTempPath()) ('JozzSplatGameLab_R0_' + [Guid]::NewGuid().ToString('N'))
        ...
        if ($plyFiles.Count -ne 1) {
            throw "R0 oczekuje dokladnie jednego PLY w ZIP-ie; znaleziono: $($plyFiles.Count)."
        }
    ...
    Write-Host ''
    Write-Host 'Uruchamiam lokalny R0 LAB...'
    Write-Host 'Pierwszy start moze chwile potrwac, bo przegladarka pobiera przypiete biblioteki renderujace.'
    Write-Host ''
    & node tools/r0-server.mjs $sourcePath
    exit $LASTEXITCODE
}
catch {
    Write-Host ''
    ...
    Show-ErrorBox ("R0: FAIL`n`n" + $_.Exception.Message + "`n`nNie naprawiaj tego sam. Wyslij mi screenshot komunikatu.")
    exit 1
}
