...
export function solveScale(measurements) {
  ...
  if (valid.length < 2) return { status: 'INSUFFICIENT', measurementCount: valid.length, required: 2, automaticAcceptance: false };
  ...
  const residuals = valid.map((item, index) => {
    ...
    return { index, impliedUnitsPerMetre, predictedSourceLength, sourceResidual, predictedMetres, metreResidual, relativeResidual, relativeResidualPct: relativeResidual * 100 };
  });
  ...
  return {
    status: 'CANDIDATE', measurementCount: valid.length, unitsPerMetre, metresPerSourceUnit, residuals,
    consistency: {
      ratioMean, ratioMedian, ratioStdDev,
